...
class Application {
  private static exposeHeaders: string[] = []
  private ENV: IEnvVariables
  private app: Express
  ...
  public async start() {
    ...
    const apolloServer = createApollo(httpServer)
    ...
    const gqlMiddleware = expressMiddleware(apolloServer, {
      context: () => ({
        dataSources: {
          jsonData,
          prisma: new PrismaClient({ datasourceUrl: this.ENV?.PGSQL_URL || '' }),
        },
      }),
    } as any)
